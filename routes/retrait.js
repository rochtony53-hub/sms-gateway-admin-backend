...
// Distingue une passerelle Comores (MVola KM) d'une passerelle Madagascar.
...
// PIN a saisir SEPAREMENT par le gateway (a l'invite operateur).
// Regle simple, sans reglage supplementaire :
//   - le modele contient {pin}     -> le PIN est integre au code (mode MVola)
...
async function buildUssd(template, numero, montant, numeroGateway, opKey) {
  ...
  // {pin} present -> on l'injecte (MVola). Absent -> PIN saisi a part (Orange).
...
function operatorNameToKeyword(opKey) {
  ...
  if (opKey === 'mvola_km') return 'MVola'; // SIM Telma Comores = "Telma/MVola"
...
async function dispatchUssdRetrait(retrait) {
  try {
    ...
    // Vide si le modele contient deja {pin} (MVola) ou si aucun PIN n'est requis.
    ...
    await Device.findByIdAndUpdate(device._id, {
      $push: {
        pendingCmds: {
          ...
          ussdCode,
          ussdPin,          // '' = PIN deja dans ussdCode (MVola) ; sinon a taper a l'invite (Orange)
          operator: opKey
        }
      }
    });
...
async function autoPollRetraitsDeriv() {
  try {
    ...
    const list = await Retrait.find({
      ...
      // GARDE-FOU ANTI-DOUBLE-PAIEMENT : ne PAS toucher les retraits de la voie
      // OTP (qui portent un derivRequestId). Ceux-la sont traites uniquement par
      // autoPollDerivRestWithdrawals. Sans cette exclusion, un retrait pouvait
      // etre dispatche par les DEUX pollers -> Mobile Money envoye deux fois.
      $or: [ { derivRequestId: null }, { derivRequestId: '' }, { derivRequestId: { $exists: false } } ]
    }).lean();
    for (const r of list) {
